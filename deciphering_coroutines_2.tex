...
\tikzset{stackframe/.style={
    ...
    % The border:
    thick,
    draw=black!80, % 50% red and 50% black,
...
IoResult inner_function() {
  ...
  \note{
  \begin{itemize}
  \item \alert<+>{aka Fibers, aka User-mode Threads}
...
\begin{frame}
  ...
  \begin{itemize}
  \item Little overhead - Simple manipulation of the function frames list making up the call stack.
  \item No synchronization needed.
  \item Green Threads cooperatively decide when to suspend and resume
...
(*@\danch(ar200)@*)Async<IoResult>(*@\danch(br200)@*) inner_function() {
  ...
  \vspace{20pt}
  % Checkmark
  \only<4>{ \tikz\fill[scale=0.4,fill=red!40!green!100](0,.35) -- (.25,0) -- (1,.7) -- (.25,.15) -- cycle; }
...
(*@\danch(ar229)@*)  Result result = r.get_result();
  ...
  \only<3-8>{\tikz[transform shape,overlay] \draw[>=triangle 45,->, darkred] ([shift={(-1em,0.25em)}]ar226.north) -- ([shift={(1em,0.25em)}]ar226.north);}
  \only<9>{\tikz[transform shape,overlay] \draw[>=triangle 45,->, darkred] ([shift={(-1em,0.25em)}]ar227.north) -- ([shift={(1em,0.25em)}]ar227.north);}
  \only<10->{\tikz[transform shape,overlay] \draw[>=triangle 45,->, darkred] ([shift={(-1em,0.25em)}]ar228.north) -- ([shift={(1em,0.25em)}]ar228.north);}
...
(*@\danch(ar235)@*)  return Result::from_partial_result(r);
}
  \end{lstlisting}
  ...
  \only<1>{\tikz[transform shape,overlay] \draw[>=triangle 45,->, darkred] ([shift={(-1em,0.25em)}]ar233.north) -- ([shift={(1em,0.25em)}]ar233.north);}
  \only<2-7>{\tikz[transform shape,overlay] \draw[>=triangle 45,->, darkred] ([shift={(-1em,0.25em)}]ar234.north) -- ([shift={(1em,0.25em)}]ar234.north);}
  \only<8->{\tikz[transform shape,overlay] \draw[>=triangle 45,->, darkred] ([shift={(-1em,0.25em)}]ar235.north) -- ([shift={(1em,0.25em)}]ar235.north);}
...
\begin{frame}
  ...
  \begin{itemize}
  \item Once a coroutine has been resumed, it cannot be suspended, unless we call \texttt{co\_await} again
  \item We do not know from the outside how often we need to resume an inner function before it succeeds
...
struct (*@\danch(ar274)@*)ResumeCaller(*@\danch(br274)@*) { /* ... */
  ...
  \item \alert<+>{***TRANSITION*** symmetric transfer!}
  \item ***TRANSITION*** overview slide
  \end{itemize}
  }
\end{frame}
...
\begin{frame}
  ...
  \begin{tikzpicture}[node distance = 0mm and 5mm]
    ...
    \node (inner)   [stackframe,above=of main,style={minimum width=80mm}]  {inner\_function()};
  ...
  \begin{tikzpicture}[node distance = 0mm and 5mm]
    \node (main)    [stackframe,fill=lightgray,style={minimum width=80mm}]     {base()};
    \node (inner)   [stackframe,above=of main,style={minimum width=80mm}]      {middle\_function()};
    \node (await)   [above=of inner,style={minimum size=6mm,minimum width=80mm}]   {};
  \end{tikzpicture}
  \end{center}
  \end{onlyenv}
...
\begin{frame}[fragile]
  ...
  \begin{lstlisting}[style=cpp20]
template<typename T>
struct (*@\danch(ar280)@*)Async(*@\danch(br280)@*) { /* ... */
  struct promise_type { /* ... */
...
(*@\danch(ar283)@*)  return Result::from_partial_result(r);
}
  \end{lstlisting}
  \mixedbox(ar280:br280)
