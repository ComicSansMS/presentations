...
\end{lstlisting}\begin{lstlisting}[style=cpp20]
  ...
  \note{
  \begin{itemize}
    \item This was our cheat sheet for the \texttt{promise\_type} from part 1. For time's sake, I will not go through it again, but we will come back to parts of it during this talk.
  \end{itemize}
  }
\end{frame}
...
\begin{frame}
  ...
  \note{
  \begin{itemize}
    \item The last piece of the puzzle is the awaitable. An awaitable is a type that we can call \texttt{co\_await} on, which is the operation that thriggers suspension.
...
\begin{frame}
  ...
  \end{itemize}
  \note{
  \color{green}{TIME!} \color{black}{0:15}
  \begin{itemize}
  \item items
...
(*@\danch(ar229)@*)  Result result = r.get_result();
  ...
  \item \alert<+>{***TRANSITION*** and is popped from the call stack}
...
\begin{frame}
  ...
  \note{
    \color{green}{TIME!} \color{black}{0:25}
    \begin{itemize}
...
\begin{frame}[fragile]
  ...
  \note{
  \color{green}{TIME!} \color{black}{0:45}
  \begin{itemize}
...
int main() {
  ...
  \item ***TRANSITION*** we can pass it along as a function argument; but this is annoying
  \end{itemize}
  }
...
void await_suspend(std::coroutine_handle<OtherPromise_T> h) {
  ...
  \item There is a design space here for where we put the type erasure; type-erased handles can't get back the promise; we won't be covering this here.
  \end{itemize}
  }
\end{frame}
...
.
  ...
  \note{
  \begin{itemize}
  \item \alert<+>{how doe we ensure the scheduler is there when we reach the first async io call? spawning lazy tasks; we initial suspend always; this ensures that when the body starts executing we already have the caller handle}
...
\begin{frame}
  ...
  \begin{itemize}
    \item Dietmar Kühl - Creating a Sender/Receiver HTTP Server
    \item Ian Petersen, Jessica Wong - How Meta Made Debugging Async Code Easier with Coroutines and Senders
    \item Dmitry Prokoptsev - Coroutines and Structured Concurrency in Practice
    \item Michael Caisse - Sender Patterns to Wrangle Concurrency in Embedded Devices
  \end{itemize}
\end{frame}
