...
\begin{frame}
  ...
  \begin{itemize}
    \pause \item They are at the heart of the error handling of many libraries, including the standard library
    \pause \item Separating error handling code from the normal execution path is often desirable
    \pause \item Handling errors originating in constructors or operators is difficult without them
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \pause \item An unbounded number of exception objects can be alive at the same time
  \pause \item Exception objects can be of arbitrary size. Allocate on the heap?
  \pause \item \texttt{std::bad\_alloc} is a common exception
  \pause \item Stack?
  \pause \item We still need to perform calls to destructors during unwinding (each of which can have their own, nested exceptions)
  \pause \item Since C++11, an exception can outlive the handler (\texttt{exception\_ptr}), so these for sure cannot be left on the stack
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \pause \item Most implementations today simply invoke malloc for allocating storage for the exception object
  \pause \item Replacing malloc can be an option for certain scenarios, requires patching the runtime
  \pause \item Is it a problem to allocate exceptions from a fixed-size arena? It might be, as that turns each \texttt{throw} and \texttt{exception\_ptr} use into a potential \texttt{terminate}
  \pause \item In practice, we already have a similar situation today with \texttt{bad\_alloc}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \pause \item Exception types are not required to match exactly
  \pause \item Polymorphic catching of exceptions requires determining the types of all base classes of an object at runtime
  \pause \item Exception class hierarchies are typically complex (one of the few places where multiple inheritance is still common)
  \pause \item There exists a solution for this problem in C++ already: RTTI
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \pause \item Traversing the Type Info structure is expensive
  \pause \item The type info structures are open for extension - pulling in a shared library can increase cost of traversals in existing code
  \pause \item Runtime analysis only possible if all types are known beforehand
  \pause \item RTTI always brings a non-zero overhead for the binary size:
    \begin{itemize}
    \pause \item Type information is stored for all types, not just the ones participating in exception handling
    \pause \item The type info structure contains more information than is needed to find the matching catch handler
    \end{itemize}
  \end{itemize}

  \pause An implementation is not required to use RTTI for exception handling. But they typically do.
\end{frame}
...
\begin{frame}
  ...
  \pause Frame-based Exception Handling

  \begin{itemize}
  \pause \item The program code actively maintains a dynamic data structure for unwinding
  \pause \item Has to keep track of active objects with non-trivial destructors and any enclosing exception handlers
  \pause \item Compiler inserts code to update these data structures accordingly while stepping through the code
  \end{itemize}

  \pause Drawbacks of this approach:
  \begin{itemize}
  \item Maintaining the data structure is costly and has to be done even if no exception ever occurs
  \pause \item Binary code size increases significantly
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \pause Table-based Exception Handling

  \begin{itemize}
  \pause \item The set of active local objects at a certain point in code is known at compile time
  \pause \item Compiler generates a table from code locations to active objects
  \pause \item During unwinding, for each stack frame use the IP as a key for a lookup into the table
  \pause \item If no exception occurs, no additional code for error handling is executed (zero-overhead exceptions)
  \end{itemize}

  \pause Drawbacks of this approach:
  \begin{itemize}
  \item Binary size increases even more to store tables
  \pause \item Table lookup for executing exceptions is very costly
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \pause \item Frame-based has better performance when exceptions occur frequently
  \pause \item Table-based has better performance if exceptions are rare
  \pause \item Both have a significant impact on binary size, even if exceptions are never used (not zero-overhead)
  \end{itemize}
\end{frame}
...
\begin{itemize}
\pause \item Memory may be limited (both RAM and ROM)
\pause \item Runtime must be predictable
\end{itemize}
...
\begin{itemize}
\pause \item RTTI is too wasteful. Prefer a different mechanism that only stores the relevant information needed for exception handling
\pause \item Restrict the types that can be used as exception types
\pause \item Use a stack unwind mechanism that favors size over speed
\pause \item Get a bigger controller
\end{itemize}

\pause Compilers so far don't seem to be very enthusiastic about supporting points 1 \& 2.

\pause Realistically, if you want to use exceptions today, you enough ROM to be able to deal with the size increase.
\end{frame}
...
\begin{itemize}
\pause \item Replace the memory management with a custom routine
\pause \item Restrict size of exception types and number of exception objects that can be active at the same time
\end{itemize}

\pause The last point is difficult to enforce, even with tool support. It might require banning \texttt{exception\_ptr}.
\end{frame}
...
\begin{itemize}
\pause \item Scanning of the type hierarchy using RTTI is difficult to reason about
\pause \item All exception types need to be known beforehand
\pause \item Polymorphism should be limited, ideally we only catch exceptions by their true dynamic type
\end{itemize}
...
\begin{itemize}
\pause \item Polymorphism is used to allow catching exceptions at different levels of generalization
\pause \item If we disallow catching by base class, equivalent logic can be implemented manually
\pause \item In practice, how much is this feature actually used?
\pause \item Restricting the error type does not imply restrictions on the amount of information that can be transported (see Boost.Exception)
\pause \item Again in practice, how much information do we actually want to transport?
\end{itemize}
