...
\date{ACCU 2025}
\titlegraphic{\includegraphics[height=.15\textheight]{resources/accu_logo.png}}
...
\begin{frame}
  ...
  \begin{center}
    \href{https://www.youtube.com/watch?v=J7fYddslH0Q}{\includegraphics[height=.80\textheight]{corogfx2/talk_deciphering_part1.jpg}}
  \end{center}
...
IoResult inner_function() {
  ...
  \note{ \color{green}{TIME!} \color{black}{0:10} }
\end{frame}
...
(*@\danch(ar430)@*)}
  ...
  \begin{onlyenv}<5-7>
  \begin{tikzpicture}[node distance = 0mm and 5mm,overlay]
    \node at([shift={(65ex,5cm)}]0,0) (main){\includegraphics[width=.07\textwidth]{corogfx/icon_pause.png}};
  \end{tikzpicture}
  \end{onlyenv}
...
\begin{frame}[fragile]
  ...
  \note{
  \color{green}{TIME!} \color{black}{0:55}
  \begin{itemize}
...
\begin{frame}[fragile]
  \frametitle{Order of events during startup}
  
  \begin{lstlisting}[style=cpp20]
Async<PartialResult> middle_function() {
(*@\danch(ar421)@*)  Async<IoResult> a = inner_function();
(*@\danch(ar423)@*)  IoResult r = co_await a;
  co_return PartialResult::from_io_result(r);
}

Async<IoResult> inner_function() {
(*@\danch(ar422)@*)  auto data = co_await async_io( /* ... */ );
  // ...
}
  \end{lstlisting}
  
  \only<1-2>{\tikz[transform shape,overlay] \draw[>=triangle 45,->, darkred] ([shift={(-1em,0.25em)}]ar421.north) -- ([shift={(1em,0.25em)}]ar421.north);}
  \only<3>{\tikz[transform shape,overlay] \draw[>=triangle 45,->, darkred] ([shift={(-1em,0.25em)}]ar422.north) -- ([shift={(1em,0.25em)}]ar422.north);}
  \only<4>{\tikz[transform shape,overlay] \draw[>=triangle 45,->, darkred] ([shift={(-1em,0.25em)}]ar423.north) -- ([shift={(1em,0.25em)}]ar423.north);}
  
  \begin{onlyenv}<1>
  \scalebox{0.75}{
  \begin{tikzpicture}[node distance=0mm and 5mm,overlay]
    \node (middle) at ([shift={(17cm,6cm)}]0,0) [stackframe,fill=orange!30] {middle\_function()};
    \node (inner)    [stackframe,above=of middle,fill=indigo!30]  {inner\_function()};
    %\node (promcons) [stackframe,above=of inner,fill=co_promise]  {promise$::$promise()};
    %\node (getret)   [stackframe,above=of promcons]  {my\_scheduler = 0;};
  ...
  \begin{tikzpicture}[node distance=0mm and 5mm,overlay]
    ...
    \node (inner)    [stackframe,above=of middle,fill=indigo!30]  {inner\_function()};
    \node (promcons) [stackframe,above=of inner,fill=co_promise]  {promise$::$promise()};
    \node (getret)   [stackframe,above=of promcons]  {sched = nullptr;};
    %\node (getret)   [stackframe,above=of middle,fill=co_promise]            {initial\_suspend()};
  \end{tikzpicture}
  }
  \end{onlyenv}
  ...
  \begin{tikzpicture}[node distance=0mm and 5mm,overlay]
    \node (middle) at ([shift={(17cm,6cm)}]0,0) [stackframe,fill=orange!30] {middle\_function()};
    \node (inner)    [stackframe,above=of middle,fill=indigo!30]  {inner\_function()};
    \node (promcons) [stackframe,above=of inner,fill=co_awaitable]  {await\_suspend()};
    \node (getret)   [stackframe,above=of promcons]  {sched->register()};
    %\node (getret)   [stackframe,above=of middle,fill=co_promise]            {initial\_suspend()};
  \end{tikzpicture}
  }
  \end{onlyenv}
  ...
  \begin{tikzpicture}[node distance=0mm and 5mm,overlay]
    \node (middle) at ([shift={(17cm,6cm)}]0,0) [stackframe,fill=orange!30] {middle\_function()};
    \node (promcons) [stackframe,above=of middle,fill=co_awaitable]  {await\_suspend()};
    \node (getret)   [stackframe,above=of promcons]  {sched = h.sched;};
    %\node (getret)   [stackframe,above=of middle,fill=co_promise]            {initial\_suspend()};
  \end{tikzpicture}
  }
  \end{onlyenv}
...
\begin{frame}
  ...
  \begin{todolist}[label=$\square$]
    ...
    \only<3>{ \item Resume inner coroutine to allow execution}
  \end{todolist}
  \end{onlyenv}
  \begin{onlyenv}<4>
  \begin{todolist}[label=$\square$]
...
(*@\danch(ar283)@*)  co_return PartialResult::from_io_result(r);
  ...
  \tikz[transform shape,overlay] \draw[>=triangle 45,->, darkred] ([shift={(-1em,0.25em)}]ar282.north) -- ([shift={(1em,0.25em)}]ar282.north);

  \scalebox{0.75}{
  \begin{tikzpicture}[node distance=0mm and 5mm,overlay]
    \node (base) at ([shift={(17cm,6cm)}]0,0) [stackframe,fill=lightgray] {base()};
    \node (outer) [stackframe,above=of base,fill=orange!30]  {middle\_function()};
...
\begin{frame}[fragile]
  ...
  \begin{onlyenv}<1-2>
  \begin{lstlisting}[style=cpp20]
struct (*@\danch(ar283)@*)Async(*@\danch(br283)@*) {
std::coroutine_handle<promise_type> (*@\danch(ar284)@*)self(*@\danch(br284)@*);
...
void await_suspend((*@\danch(ar285)@*)std::coroutine_handle<OtherPromise_T> h(*@\danch(br285)@*)) {
  ...
  \monobox[green](ar283:br283)
  \only<2>{ \monobox[indigo](ar284:br284) }
  \only<2>{ \monobox[orange](ar285:br285) }
...
(*@\danch(ar283)@*)  co_return PartialResult::from_io_result(r);
  ...
  \begin{tikzpicture}[node distance=0mm and 5mm,overlay]
    \node (base) at ([shift={(17cm,6cm)}]0,0) [stackframe,fill=lightgray] {base()};
    \node (outer) [stackframe,above=of base,fill=orange!30]  {middle\_function()};
    \node (middle)   [stackframe,above=of outer,fill=indigo!30]              {inner\_function()};
    \node (getret)   [stackframe,above=of middle,fill=co_promise]            {initial\_suspend()};
  \end{tikzpicture}
  }
  \end{onlyenv}
  ...
  \begin{tikzpicture}[node distance=0mm and 5mm,overlay]
    \node (base) at ([shift={(17cm,6cm)}]0,0) [stackframe,fill=lightgray] {base()};
    \node (outer) [stackframe,above=of base,fill=indigo!30]  {inner\_function()};
  \end{tikzpicture}
  }
  \end{onlyenv}
  ...
  \note{
  \begin{itemize}
  \item \alert<+>{execute again, inner is on the stack, but we don't enter body because of initial\_suspend}
  \item \alert<+>{***TRANSITION*** back in middle we co\_await on the async returned by inner}
  \item \alert<+>{***TRANSITION*** await\_suspend initiates the symmetric transfer}
  \item ***TRANSITION*** now we enter body of inner; middle is suspended and no longer on stack!
  \end{itemize}
  }
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{lstlisting}[style=cpp20]
(*@\danch(ar461)@*)Async<Result>(*@\danch(br461)@*) outer_function();

int main() {
  (*@\danch(ar462)@*)Async<Result>(*@\danch(br462)@*) r = outer_function();
  Result result = /* ??? */;
}
  \end{lstlisting}
  \mixedbox(ar461:br461)
  \monobox[blue](ar462:br462)
  
  \note{ \color{green}{TIME!} \color{black}{0:75} }
\end{frame}

\begin{frame}
  \frametitle{Interface of \texttt{Async}}
  \begin{itemize}
  \item Starts suspended (lazy)
  \item Expects execution context before executing body
  \item Expects handle to an outer coroutine to transfer to after completing
  \item Initially resumed via symmetric transfer
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \only<1>{\includegraphics[width=\textwidth]{corogfx2/async_call_stack_0.png}}
  \only<2>{\includegraphics[width=\textwidth]{corogfx2/async_call_stack.png}}
