...
\title{Dynamic Memory on Embedded Systems}
%\subtitle{}
\author{Andreas Weis}
\institute{BMW AG}

\date{emBO, March 15, 2019}
...
\begin{frame}
  \frametitle{Overview}
  \begin{itemize}
  \item Static and dynamic memory management
...
\begin{frame}[fragile]
  \frametitle{The embedded way}

  What's wrong with\ldots

  \begin{lstlisting}
    static int data_buffer[256];
  \end{lstlisting}

  \begin{itemize}
    \pause \item In embedded often the only dynamic memory management is the stack
    \pause \item Out-of-memory errors surface during build or flashing
    \pause \item Extremely rigid and does often not scale well to big projects
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{The case for dynamic memory}

  \begin{itemize}
  \item Dynamic memory is an enabler
    \begin{itemize}
    \item Libraries (C++ standard library)
    \item Advanced language features
    \item Algorithms
    \end{itemize}
  \item It can make things easier
    \begin{itemize}
    \item Heavy reuse of memory buffers
    \item Projects with a generous memory budget
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}
  \frametitle{Be careful with extending the algorithm\ldots}
...
\begin{frame}[fragile]
  \frametitle{Multipool Allocator}
  \begin{center}
    \includegraphics[width=.9\textwidth]{memgfx/multipool_030.png}
  \end{center}

  \begin{semiverbatim}
    {\color{blue}auto} p1 = multipool.allocate(6);
    {\color{blue}auto} p2 = multipool.allocate(2);
  \end{semiverbatim}
\end{frame}
...
\begin{frame}
  ...
  \item For embedded projects a simpler solution is often a better fit
